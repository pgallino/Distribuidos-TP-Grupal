...
class GenreFilter(Node):
    ...
    def _process_games_message(self, msg):
        ...
        if indie_basic_games:
            indie_basic_msg = ListMessage(type=MsgType.GAMES, item_type=GamesType.BASICGAME, items=indie_basic_games, client_id=msg.client_id, msg_id=msg.msg_id)
            self._middleware.send_to_queue(E_FROM_GENRE, indie_basic_msg.encode(), key=K_INDIE_BASICGAMES)

        if indie_q2_games:
            indie_q2_msg = ListMessage(type=MsgType.GAMES, item_type=GamesType.Q2GAMES, items=indie_q2_games, client_id=msg.client_id, msg_id=msg.msg_id)
            self._middleware.send_to_queue(E_FROM_GENRE, indie_q2_msg.encode(), key=K_INDIE_Q2GAMES)
        
        if shooter_games:
            shooter_msg = ListMessage(type=MsgType.GAMES, item_type=GamesType.BASICGAME, items=shooter_games, client_id=msg.client_id, msg_id=msg.msg_id)
